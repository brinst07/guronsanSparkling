...
$(function() {
	...
	$('#freeboardBody tr').on('click', function() {
		...
		library_no = $(this).find('td:eq(0) input').val();
		
		rnum = $(this).find('td:eq(0)').text();
		$(location).attr('href', '${pageContext.request.contextPath}/user/library/libraryView.do?library_no=' + library_no);
	});
			...
			<c:if test="${!empty libraryList }">
				<c:forEach items="${libraryList }" var="libraryInfo">
					<tr>
						...
						<td>${libraryInfo.library_nickname }</td>
						<td>${libraryInfo.library_reg_date }</td>
						<td>${libraryInfo.library_hit }</td>
					</tr>
				</c:forEach>
			</c:if>
			</tbody>			
